...
plot(c(0),((1 - pAtm[[1]]$V7)*pAtm[[1]]$V9), xlim = range(c(0,1000)), ylim =
range(c(0.0094,0.0098)), ylab = "Excess CO2 mass", xlab = "Mass of added
fluids")

for (i in 1:length(pAtm)) {
    points(i, ((1 - pAtm[[i]]$V7)*pAtm[[i]]$V9), add=TRUE, cex = 0.01)}
...
for (i in 1:length(p100)) {
    points(i, ((1 - p100[[i]]$V7)*p100[[i]]$V9), add=TRUE, cex = 0.01, col = "red")}
...
for (i in 1:length(p200)) {
    points(i, ((1 - p200[[i]]$V7)*p200[[i]]$V9), add=TRUE, cex = 0.01, col = "green")}

legend(0,0.0098, c("Surface","100 MPa","200 MPa"), col=c("black","red","green"), lty=1:2, cex=0.8)
